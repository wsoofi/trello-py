...
class Boards(object):
    ...
    def add_member(self, board_id, member_id, type):
        resp = requests.put("https://trello.com/1/boards/%s/members/%s" % (board_id, member_id), params=dict(key=self._apikey, token=self._token), data=dict(type=type))
        resp.raise_for_status()
        return json.loads(resp.text)
