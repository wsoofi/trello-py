...
class Organizations(object):
    ...
    def add_member(self, org_id_or_name, member_id, type): #WS add
        resp = requests.put("https://trello.com/1/organizations/%s/members/%s" % (org_id_or_name, member_id), params=dict(key=self._apikey, token=self._token), data=dict(type=type))
        resp.raise_for_status()
        return json.loads(resp.text)
