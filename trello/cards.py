...
class Cards(object):
    ...
    def get_action(self, card_id, filter=None, fields=None, limit=None, page=None, since = None, idModels=None):
        resp = requests.get("https://trello.com/1/cards/%s/actions" % (card_id), params=dict(key=self._apikey, token=self._token, filter=filter, fields=fields, limit=limit, page=page, since = since, idModels=idModels), data=None)
        resp.raise_for_status()
        return json.loads(resp.text)
